...
public class HttpRequestGetAllOffers extends AsyncTask<Void, Void, List<ManningOffer>> {
	
	private Activity activity;
	private static int lastOfferView;
	
	public HttpRequestGetAllOffers(Activity activity) {
		this.activity = activity;
		this.lastOfferView = 0;
	}
	
	@Override
	protected List<ManningOffer> doInBackground(Void... params) {
		
		List<ManningOffer> manningOffers = null;
		
		try {
			final String url = "http://manningcatcherws-gevapps.rhcloud.com/gevapp/manningCatcher/list";
